...
책은 세 꼴이 있다.
  ...
  · 바깥 링크("external": true) — 복사할 파일이 없고 links 의 주소만 목록에 건다
    (예: 로그인 없이 열리는 Google Slides). src 도 없어야 한다.
...
def main() -> None:
    ...
    for b in books:
        ...
        if b.get("external"):
            links = [f'<a href="{html.escape(l["url"])}" target="_blank" rel="noopener">'
                     f'{html.escape(l["label"])}</a>' for l in b["links"]]
        else:
